...
MODIFIED (2025-04-15): Added debug logging for session state changes, rounded percentage inputs,
                       added input change warning, stored inputs per run, added negative cash flow warning,
                       logged NOI components, and implemented simulation caching.
"""
...
def main():
    ...
    for key in all_input_keys:
        ...
        try:
            if key in percentage_decimal_keys:
                processed_inputs_dict[key] = round(float(ui_value) / 100.0, 4)  # Round to 4 decimals
            elif key in percentage_direct_keys:
                processed_inputs_dict[key] = round(float(ui_value), 4)  # Round to 4 decimals
    ...
    # Cache simulation to improve performance
    @st.cache_data
    def cached_monte_carlo(inputs_dict, num_sims, forward_curve_hash, std_dev_hash):
        inputs = SimulationInputs(**inputs_dict)
        return run_monte_carlo(inputs, num_sims, forward_curve_data, std_dev_curve_data)

    if run_sim_button:
        logger.info("Run Simulation button clicked.")
        if st.session_state.get("processed_results") and not all(
            np.isclose(
                st.session_state["inputs"].get(k, np.nan),
                st.session_state["processed_results"].get("inputs_used", {}).get(k, np.nan),
                rtol=FLOAT_RTOL, atol=FLOAT_ATOL, equal_nan=True
            ) if isinstance(st.session_state["inputs"].get(k), float)
            else st.session_state["inputs"].get(k) == st.session_state["processed_results"].get("inputs_used", {}).get(k)
            for k in all_input_keys
        ):
            st.warning("⚠️ Sidebar inputs have changed since the last simulation. Results may differ.")
        st.session_state["processed_results"] = None
        ...
        if inputs_valid_for_sim:
            with st.spinner(f"Running {num_sims_run} simulations..."):
                forward_hash = hash(str(forward_curve_data))
                std_dev_hash = hash(str(std_dev_curve_data))
                mc_results = cached_monte_carlo(st.session_state["inputs"], num_sims_run, forward_hash, std_dev_hash)
            if mc_results is None or 'error' in mc_results:
                ...
                mc_results["inputs_used"] = st.session_state["inputs"].copy()
    ...
    if processed_results and "error" in processed_results:
        ...
        with tabs[tab_keys.index("📊 Summary")]:
            ...
            if inputs_used_for_run and inputs_valid_for_display:
                try:
                    ...
                    logger.info(f"Initial NOI Calc: PGR={potential_gross_rent:,.0f}, Vacancy={vacancy_amount:,.0f}, EGR={effective_gross_rent:,.0f}, OtherInc={other_income_val:,.0f}, EGI={effective_gross_income:,.0f}, OpEx={op_ex:,.0f}, NOI={net_operating_income:,.0f}")
                    ...
                    if not np.isfinite(interest_yr1):
                        interest_yr1 = 0.0
                    if not np.isfinite(principal_yr1):
                        principal_yr1 = 0.0
                    total_debt_service_yr1 = interest_yr1 + principal_yr1
                    levered_cf_yr1_approx = unlevered_cf_yr1_approx - total_debt_service_yr1
                    initial_equity = inputs_used_for_run.initial_equity
                    levered_cash_yield = levered_cf_yr1_approx / initial_equity if initial_equity > FLOAT_ATOL else np.nan

                    if levered_cf_yr1_approx < 0:
                        st.warning("⚠️ Year 1 Levered Cash Flow is negative. Consider reducing loan amount, interest rate, or expenses, or increasing NOI.")

                    col_is1, col_is2 = st.columns(2)
                    with col_is1:
                        st.metric("Purchase Price", f"${inputs_used_for_run.purchase_price:,.0f}")
                        st.metric("Potential Gross Rent (PGR)", f"${potential_gross_rent:,.0f}")
                        st.metric(f"Less: Vacancy ({inputs_used_for_run.current_vacancy:.1%})", f"(${vacancy_amount:,.0f})")
                    ...
                    with col_is2:
                        ...
                        st.markdown("---")
                        st.metric(f"Loan Amount ({inputs_used_for_run.loan_to_cost:.0%})", f"${loan_amount_val:,.0f}")
        ...
        with tabs[tab_keys.index("📈 IRR")]:
            ...
            if unlev_failures > 0:
                unlev_title += f" — {unlev_failures} failures"
            if finite_unlevered:
                fig_unlevered = plot_irr_distribution(
                    irr_values=finite_unlevered, title=unlev_title, mean_irr=mean_unlevered_irr,
                    median_irr=median_unlevered_irr, p05_irr=p05_unlevered_irr, p95_irr=p95_unlevered_irr,
                    color='blue', percent_format=True
                )
            ...
            if lev_failures > 0:
                lev_title += f" — {lev_failures} failures"
            if finite_levered:
                st.markdown("#### Levered IRR Distribution (Full View)")
                fig_levered_full = plot_irr_distribution(
                    irr_values=finite_levered, title=lev_title, mean_irr=mean_l_irr,
                    median_irr=median_l_irr, p05_irr=p05_l_irr, p95_irr=p95_l_irr,
                    color='orange', percent_format=True
                )
            ...
            if not avg_cash_flow_data or not avg_cash_flow_data.get("noi") or len(avg_cash_flow_data["noi"]) != hold_period_actual or any(pd.isna(x) for x in avg_cash_flow_data["noi"]):
                ...
                if refi_year:
                    ...
                    if noi_at_refi > 0 and refi_cap_rate > FLOAT_ATOL:
                        ...
                        logger.info(f"Refinance Year {refi_year}: Value={property_value:,.0f}, NewLoan={new_loan_amount:,.0f}, ProceedsNet={refi_proceeds_net:,.0f}")
                    else:
                ...
                # Warn if levered cash flows are negative
                levered_cf = proforma_df.loc["Levered Cash Flow (IRR)"].astype(float).tolist()
                if any(cf < 0 for cf in levered_cf[1:-1]):
                    st.warning("⚠️ Levered cash flows are negative in some years. Review NOI, debt terms, or expenses.")
                ...
                if finite_levered_plot_data:
                    try:
                        ...
                        if np.isfinite(mean_filtered):
                            fig_box.add_trace(go.Scatter(x=['Levered IRR Dist.'], y=[mean_filtered], mode='markers', marker=dict(color='red', size=8, symbol='cross-thin'), name='Mean (Filtered)', hoverinfo='skip'))
                        ...
                        if num_excluded > 0:
                            st.caption(f"Note: {num_excluded} extreme outlier(s) excluded from plot.")
                    except Exception as e:
        ...
        with tabs[tab_keys.index("🚪 Exit")]:
            ...
            if not finite_exit_values or not finite_exit_caps:
                ...
                mean_exit_val = metrics.get('mean_exit_value', np.nan)
                median_exit_val = metrics.get('median_exit_value', np.nan)
                p05_exit_val = metrics.get('p05_exit_value', np.nan)
                p95_exit_val = metrics.get('p95_exit_value', np.nan)
                mean_exit_cap = metrics.get('mean_exit_cap', np.nan)
                median_exit_cap = metrics.get('median_exit_cap', np.nan)
                ...
                try:
                    hist_vals, bin_edges = np.histogram(finite_exit_values, bins=30)
                    ...
                    fig_exit_val.update_layout(
                        title=f"Distribution of Net Exit Values ({len(finite_exit_values)} runs)",
                        xaxis_title="Net Exit Value ($)", yaxis_title="Frequency", bargap=0.1, template="plotly_white"
                    )
                    fig_exit_val.update_xaxes(tickformat="$,.0s")
                    st.plotly_chart(fig_exit_val, use_container_width=True)
                except Exception as e:
                    logger.error(f"Error plotting exit value distribution: {e}")
                ...
                p05_cap = np.percentile(finite_exit_caps, 5) if finite_exit_caps else np.nan
                p95_cap = np.percentile(finite_exit_caps, 95) if finite_exit_caps else np.nan
                ...
                try:
                    ...
                    hist_caps, bin_edges_caps = np.histogram(valid_exit_caps_pct, bins=30)
                    ...
                    fig_exit_cap.update_layout(
                        title=f"Distribution of Simulated Exit Cap Rates ({len(finite_exit_caps)} runs)",
                        xaxis_title="Exit Cap Rate (%)", yaxis_title="Frequency", bargap=0.1, template="plotly_white"
                    )
                    fig_exit_cap.update_xaxes(ticksuffix="%")
                    st.plotly_chart(fig_exit_cap, use_container_width=True)
                except Exception as e:
                    logger.error(f"Error plotting exit cap distribution: {e}")
        ...
        with tabs[tab_keys.index("🔎 Sensitivity")]:
            ...
            param_options = {
                ...
                "Prob. Normal → Recession (%)": ("transition_normal_to_recession", "
