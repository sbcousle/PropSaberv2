...
and aggregating their values. Extracted and adapted from the main script.MODIFIED: Added validation in _render_refinancing_inputs to check
...
from propsaber.core.utils import simulation_error_handler# Use relative imports for components within the propsaber package
...
logger = logging.getLogger(__name__)# --- Helper to Get Input Value from Session State ---
...
def _render_property_inputs(default_inputs: SimulationInputs):
    ...
    with st.expander(" Property Info", expanded=False):
        st.number_input(
            ...
            help="Total acquisition price of the property."
        )
        st.number_input(
...
def _render_simulation_setup_inputs(default_inputs: SimulationInputs):
    ...
    with st.expander(" Simulation Control", expanded=False):
        # Hold Period (Slider only, max 30) - UPDATED MAX VALUE
        st.slider(
            "Hold Period (Years)",
            min_value=1, max_value=30, step=1, # Increased max_value
            value=int(get_input_value("hold_period", default_inputs.hold_period)),
            key="input_hold_period", # <<< Key used for validation later
            help="Investment hold period in years (up to 30 years)."
        )

    # Number of Simulations (Number input only)
    num_sim_help = f"""
    ...
    st.number_input(
        ...
        step=100, key="input_num_simulations", format=FMT_INTEGER,
        help=num_sim_help
    )

    # Cyclical Persistence (Slider only)
    ...
    * **Default (0.5):** Moderate persistence.
    """
    st.slider(
        "Cyclical Persistence (Growth)", min_value=0.0, max_value=1.0, step=0.05, # Changed max to 1.0
        value=float(get_input_value("growth_persistence_phi", default_inputs.growth_persistence_phi)),
        help=growth_persistence_help,
        key="input_growth_persistence_phi", # Key for sidebar aggregation
...
def _render_rent_inputs(default_inputs: SimulationInputs):
    ...
    with st.expander(" Rent & Market Regimes", expanded=False):
        st.number_input(
            ...
            step=10.0, key="input_base_rent", format=FMT_CURRENCY_ZERO_DP,
    ...
    * **Zero (0%)**: Initial Rent starts exactly at Fair Value.
    ...
    st.slider(
        ...
        # Convert internal decimal (e.g., -0.05) to percentage for display
    ...
    **Note:** This should typically be *higher* than the overall long-term historical average for the market,
    ...
    st.slider(
        ...
        # This value is stored directly as %, no conversion needed for display
    ...
    """
    st.slider(
        "Prob. Normal → Recession (%/Yr)", min_value=0.0, max_value=100.0, step=1.0,
        # Convert internal decimal (e.g., 0.0) to percentage for display
    ...
    st.slider(
        ...
        min_value=-10.0, max_value=5.0, step=0.1,
    ...
    """
    st.slider(
        "Prob. Recession → Normal (%/Yr)", min_value=0.0, max_value=100.0, step=1.0,
...
def _render_vacancy_inputs(default_inputs: SimulationInputs):
    ...
    with st.expander(" Vacancy", expanded=False):
        st.slider(
            "Starting Vacancy Rate (%)", min_value=0.0, max_value=50.0, step=0.1,
        ...
        st.slider(
            "Target Long-Term Vacancy Rate (%)", min_value=0.0, max_value=30.0, step=0.1,
    ...
    """
    st.slider(
        "Vacancy Rate Volatility (% pts/Yr)", min_value=0.0, max_value=5.0, step=0.1,
...
def _render_other_income_inputs(default_inputs: SimulationInputs):
    ...
    with st.expander(" Other Income", expanded=False):
        st.number_input(
            ...
            step=1000.0, format=FMT_CURRENCY_ZERO_DP, key="input_mean_other_income",
            help="Starting annual income from sources other than rent (e.g., parking, fees, laundry)."
            )
        st.slider(
            "Avg Other Income Growth (%/Yr)", min_value=-5.0, max_value=10.0, step=0.1,
        ...
        st.slider(
            "Other Income Growth Volatility (% pts)", min_value=0.0, max_value=10.0, step=0.1,
...
def _render_opex_inputs(default_inputs: SimulationInputs):
    ...
    with st.expander(" Operating Expenses", expanded=False):
        st.number_input(
            ...
            step=10000.0, format=FMT_CURRENCY_ZERO_DP, key="input_mean_expense",
            help="Starting total annual operating expenses (excluding CapEx and debt service)."
            )
        st.slider(
            "Average OpEx Growth (%/Yr)", min_value=0.0, max_value=10.0, step=0.1,
        ...
        st.slider(
            "OpEx Growth Volatility (% pts)", min_value=0.0, max_value=10.0, step=0.1,
...
def _render_capex_inputs(default_inputs: SimulationInputs):
    ...
    with st.expander(" Capital Expenditures (CapEx)", expanded=False):
        st.number_input(
            ...
            step=25.0, key="input_capex_per_unit_yr", format=FMT_CURRENCY_ZERO_DP,
            help="Starting annual capital reserve budget per unit."
            )
        st.slider(
            "Average CapEx Growth (%/Yr)", min_value=0.0, max_value=10.0, step=0.1,
        ...
        st.slider(
            "CapEx Growth Volatility (% pts)", min_value=0.0, max_value=10.0, step=0.1,
...
def _render_exit_inputs(default_inputs: SimulationInputs):
    ...
    with st.expander(" Exit Assumptions", expanded=False):
        st.slider(
            "Average Exit Cap Rate (%)", min_value=1.0, max_value=10.0, step=0.05,
        ...
        st.slider(
            "Exit Cap Rate Volatility (% pts)", min_value=0.0, max_value=3.0, step=0.05,
            ...
            format=FMT_PERCENT_TWO_DP # Format as percentage points
        )
    ...
    * **Zero:** No adjustment based on terminal rent growth.
    ...
    st.slider(
        "Transaction Cost on Sale (%)", min_value=0.0, max_value=5.0, step=0.1,
    ...
    st.number_input(
        ...
        step=100000.0, format=FMT_CURRENCY_ZERO_DP, key="input_exit_floor_value",
...
def _render_financing_inputs(
    ...
    forward_curve: Optional[Dict[int, float]], # Pass curve data as argument
    std_dev_curve: Optional[Dict[int, float]] # Pass std dev data as argument
    ):
    """
    Renders input widgets for financing parameters.
    Requires forward curve data to be passed in for preview.
    """
    with st.expander(" Financing", expanded=False):
        st.slider(
            "Loan-to-Cost Ratio (%)", min_value=0.0, max_value=90.0, step=1.0,
        ...
        # --- ADDED: Initial Loan Costs ---
        st.slider(
            "Initial Loan Costs (% of Loan)", min_value=0.0, max_value=5.0, step=0.1,
    ...
    if rate_type == "Fixed":
        st.slider(
            "Fixed Interest Rate (%/Yr)", min_value=0.0, max_value=12.0, step=0.1,
        ...
        if not forward_curve or not std_dev_curve:
            ...
            st.slider(
                "Spread Over SOFR (%/Yr)", min_value=0.0, max_value=5.0, step=0.1,
            ...
            st.slider(
                "SOFR Floor (%/Yr)", min_value=0.0, max_value=5.0, step=0.1,
            ...
            * **0.0 = No random rate volatility (rate follows forward curve + persistence only).**
            ...
            st.slider(
                ...
                step=0.1, key="input_volatility_scalar",
                help=volatility_scalar_help, format=FMT_DECIMAL_TWO_DP
            )
...
def _render_refinancing_inputs(default_inputs: SimulationInputs):
    ...
    ADDED: Validation check for refi_year vs hold_period.
    """
    ...
    # Read current state of checkbox to conditionally display other options AND perform validation
    # Use widget keys directly as they reflect the most up-to-date user interaction
    enable_refi_state = st.session_state.get("input_enable_refinancing", default_inputs.enable_refinancing)

    if enable_refi_state:
        # Refinance Year Slider
        st.slider(
            "Refinance Year",
            min_value=1, max_value=29, # Allow refi up to year before max hold
            value=int(get_input_value("refi_year", default_inputs.refi_year)),
            step=1, key="input_refi_year", # <<< Key used for validation
            format=FMT_INTEGER,
            help="The specific year during the hold period when the refinance occurs."
        )

        # --- ADDED VALIDATION ---
        # Get current values from sliders using their keys
        ...
        # Other Refinancing Inputs (remain the same)
        refi_rate_help = """
        ...
        st.slider(
            ...
            format=FMT_PERCENT_TWO_DP + "%%" # Display as %
        )
        refi_ltv_help = """
        ...
        st.slider(
            "New Loan: Target LTV (%)",
            min_value=50.0, max_value=85.0, step=0.5,
...
def _render_correlation_inputs(default_inputs: SimulationInputs):
    ...
    with st.expander(" Correlation", expanded=False):
        st.checkbox(
            ...
            key="input_use_correlations", # Read from this key below
    ...
    if use_corr_state:
        ...
        """
        st.slider(
            "Correlation: Rent & Expense Shocks", min_value=-1.0, max_value=1.0, step=0.05,
        ...
        """
        st.slider(
            "Correlation: Rent & Other Income Shocks", min_value=-1.0, max_value=1.0, step=0.05,
        ...
        """
        st.slider(
            "Correlation: Rent & Vacancy Shocks", min_value=-1.0, max_value=1.0, step=0.05,
...
def _render_risk_metric_inputs(default_inputs: SimulationInputs):
    ...
    with st.expander(" Risk Metric Settings", expanded=False):
        st.slider(
            "Risk-Free Rate (%/Yr)", min_value=0.0, max_value=10.0, step=0.1,
        ...
        st.slider(
            "Hurdle Rate (% IRR)", min_value=0.0, max_value=25.0, step=0.5,
...
def render_sidebar_inputs(
    ...
    ) -> None: # Modified to return None, as state is managed via st.session_state
    """
    Renders all input sections in the Streamlit sidebar.
    Relies on st.session_state['inputs'] being initialized and updated.
    Uses widget keys prefixed with 'input_' (e.g., 'input_purchase_price').
    """
...
_render_refinancing_inputs(defaults) # Contains the new validation
